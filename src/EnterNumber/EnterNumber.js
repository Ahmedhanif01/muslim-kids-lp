...
function EnterNumber({ state, dispatch }) {
  ...
  const sendPin = async (telco, number) => {
    ...
    isHE
      ...
      isHE ? facebookPixelEvents( "he_first_button_pressed") : facebookPixelEvents("send_pin_pressed")
    if (isHE) {
      try {
        let url;
        ...
        if (resp.status === 1) {
          if (state.flow === "SINGLE_CLICK") {
            ...
            facebookPixelEvents("he_first_button_single_click_pressed_success", { ...resp });
  ...
  return (
    <div className="input-form">
      <>
        <div className="label">
          {lang === "en" && <img src={getCDNUrl("phone.svg")} alt="" />}
          {language[lang].enter_number}
          {lang !== "en" && <img src={getCDNUrl("phone.svg")} alt="" />}
        </div>
        <div className="input">
          <input

...
const telenor_pakistan_daily = 12; // telenor service id
...
// pin length
const TELENOR_PAKISTAN_WEEKLY_PIN_LENGTH = 5;
const ZONG_PAKISTAN_DAILY_PIN_LENGTH = 5;
const UFONE_PAKISTAN_DAILY_PIN_LENGTH = 4;
// const TELENOR_PAKISTAN_FREE_WEEKLY_PIN_LENGTH = 5;
...
class ConfigManager {
    ...
    pinflow = "http://cpav2.knectapi.com/index.php/knectapi/";
    configuration = {
        pk: {
            services: [
                {
                    ...
                    header: {
                        getNumberUrl: () => {
                            ...
                            return `${this.cricwick}checkhe?serviceId=${this.configuration.pk.services[0].SERVICE_ID}&partnerId=${partner_id ? partner_id : this.configuration.pk.services[0].PARTNER_ID}`
                        },
                        singleClick:
                        {
                            getUrl: ({ token, ip, subSrc }) => {
                                let partner_id = getParametrByName("partner_id");
                                return `${this.cricwick}single_click?token=${token}&serviceId=${this.configuration.pk.services[0].SERVICE_ID}&partnerId=${partner_id ? partner_id : this.configuration.pk.services[0].PARTNER_ID}&userIp=${ip}&subSource=${subSrc}&campaignId=${this.configuration.pk.services[0].CAMPAIGN_ID}`
                            }
                        }
                        ,
                        clicks: [
                            {
                                getUrl: ({ token, ip, subSrc }) => {
                                    let partner_id = getParametrByName("partner_id");
                                    return `${this.cricwick}header_click1?token=${token}&serviceId=${this.configuration.pk.services[0].SERVICE_ID}&partnerId=${partner_id ? partner_id : this.configuration.pk.services[0].PARTNER_ID}&userIp=${ip}&sourceId=${this.configuration.pk.services[0].SOURCE_ID}&subSource=${subSrc}&campaignId=${this.configuration.pk.services[0].CAMPAIGN_ID}`
                                }
                            },
                            {
                                getUrl: ({ token, number, userAgent }) => {
                                    let partner_id = getParametrByName("partner_id");
                                    return `${this.pinflow}pinless_verifytoken?service_id=${this.configuration.pk.services[0].SERVICE_ID}&partner_id=${partner_id ? partner_id : this.configuration.pk.services[0].PARTNER_ID}&token=${token}&msisdn=${number}&user_agent=${userAgent}`;
                                }
                            }
                        ]
                    },
                    pinFlow: {
                        getSendPinUrl: ({ ip, subSrc, number, userAgent }) => {
                            let partner_id = getParametrByName("partner_id");
                            return `${this.pinflow}send_pin?service_id=${this.configuration.pk.services[0].SERVICE_ID}&partner_id=${partner_id ? partner_id : this.configuration.pk.services[0].PARTNER_ID}&sub_source=${subSrc}&campaign_id=${this.configuration.pk.services[0].CAMPAIGN_ID}&user_ip=${ip}&msisdn=${number}&user_agent=${userAgent}`;
                        },
                        getConfirmPinUrl: ({ ip, subSrc, number, userAgent, pin }) => {
                            let partner_id = getParametrByName("partner_id");
                            return `${this.pinflow}confirm_pin?service_id=${this.configuration.pk.services[0].SERVICE_ID}&partner_id=${partner_id ? partner_id : this.configuration.pk.services[0].PARTNER_ID}&sub_source=${subSrc}&campaign_id=${this.configuration.pk.services[0].CAMPAIGN_ID}&user_ip=${ip}&msisdn=${number}&pincode=${pin.trim()}&user_agent=${userAgent}`;
                        },
                        isMcpEnabled: false,
                        mcpScript: () => ``
                    },
            ...
            price_points: {
                en: () => {
                    ...
                    if (allPaths.includes('telenor')) {
                        return "Service charges PKR 4.18 (tax inclusive) per day to be deducted from mobile balance.";
                    } else
                        return `Telenor: Service charges PKR 4.18 (tax inclusive) per day to be deducted from mobile balance.
                        \nZong: Service charges PKR PKR 4.18 (tax inclusive) per day to be deducted from mobile balance.
                        \nUfone: PKR 4 (tax inclusive) per day to be deducted from mobile balance.
        ...
        ur: () => {
          ...
          if (allPaths.includes("telenor")) {
            ...
            return `Telenor: سروس چارجز PKR 4.18 (ٹیکس سمیت) یومیہ موبائل بیلنس سے کاٹے جائیں گے۔
            \nZong: سروس چارجز PKR 4.18 (ٹیکس سمیت) یومیہ موبائل بیلنس سے کاٹے جائیں گے۔
                        \nUfone: PKR 4 (بشمول ٹیکس) فی دن موبائل بیلنس سے کاٹا جائے گا۔
      ...
      getValidServices: () => {
        ...
        // }
        // return this.configuration.pk.services.filter(services => services.defaults === true);
        return [telenor_pakistan_daily, ufone_pakistan_daily, zong_pakistan_daily];
      },
    },
  };
